...
class GloVe(nn.Module):
    def __init__(self, v_size: int, e_dim: int, xmax: int, alpha: float):
        super().__init__()
        self.w = nn.Embedding(num_embeddings=v_size, embedding_dim=e_dim)

        self.w_ = nn.Embedding(num_embeddings=v_size, embedding_dim=e_dim)

        self.b = nn.Parameter(torch.randn(v_size, dtype=torch.float))

        self.b_ = nn.Parameter(torch.randn(v_size, dtype=torch.float))
        self.xmax = xmax
        self.alpha = alpha
